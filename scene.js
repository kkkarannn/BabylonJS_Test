...
var createScene = function () {
    ...
    var camera = new BABYLON.ArcRotateCamera("Camera", 5 * Math.PI / 2, Math.PI ,20, new BABYLON.Vector3(10, 0, 0), scene);
    camera.radius = 20;
	
    // The goal height of camera above local origin (centre) of target
    camera.heightOffset = 00;
    
    // The goal rotation of camera around local origin (centre) of target in x y plane
    camera.rotationOffset = 00;
    ...
    case BABYLON.PointerEventTypes.POINTERDOWN:
      if(pointerInfo.pickInfo.hit) {
        camera.setTarget(pointerInfo.pickInfo.pickedMesh);
...
var createVideoModel = function (scene, streamId, isLocal) {
  ...
  if (streamCount <= 1) {
    ...
    videoBackground.position = new BABYLON.Vector3(10, 0, 0);
    //videoBackground.rotation = new BABYLON.Vector3(0, 180, 0);
  } else {
    videoBackground.position = new BABYLON.Vector3(10*streamCount, 0, 0);
    //videoBackground.rotation = new BABYLON.Vector3(0, 180, 0);
  }
...
// Agora settings
const agoraAppId = '8230a2f9fb5e4e5c8aae14f1af415456'; // insert Agora AppID here
